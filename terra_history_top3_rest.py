#!/usr/bin/env python3
"""Stage or promote all remaining quote-bearing 1931--1995 top-three books."""
...
SELECTION = batch.DATA / "sample_top3_1931_1995_rest_labelable.csv"
...
def build_selection() -> None:
    ...
    classified = {
        ...
        for path in glob.glob(str(batch.DATA / "classified_top3_1931_1995*" / "*.json"))
    }
    rows = [row for row in csv.DictReader(SOURCE_SAMPLE.open(encoding="utf-8"))
            if row["sample_id"] in fetched and row["sample_id"] in quote_ids
            and row["sample_id"] not in classified]
    with SELECTION.open("w", encoding="utf-8", newline="") as file:
        writer = csv.DictWriter(file, fieldnames=rows[0].keys())
        writer.writeheader()
        writer.writerows(rows)
    print(f"prepared {len(rows)} remaining historical classification jobs")
...
batch.CLASSIFIED = batch.DATA / "classified_top3_1931_1995_rest"
batch.RUN_ID = "20260814_terra_history_top3_rest_v1"
